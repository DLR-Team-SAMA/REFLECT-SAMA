...
from utils import ConvLogger

### Configs
last_image_only = False
...
text = '''A task plan is being executed in AI2Thor simulation environment:
            ...
            The following is the execution of the substask - '''
...
for tsk,tsk_fld in zip(sub_tasks,sub_task_flds):
    ...
    print(response.text)
    log_txt += response.text+'\n'
    print("#"*50)
    log_txt += "#"*50+'\n'

f = open('data/logs/log_no.txt', 'r')
log_no = int(f.read())
f.close()
log_no += 1

with open('data/logs/log_no.txt', 'w') as f:
    f.write(str(log_no))

with open('data/logs/log_'+str(log_no)+'.txt', 'w') as f:
    f.write(log_txt)